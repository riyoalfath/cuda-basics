...
// __global__ defines this as a CUDA kernel, a function that runs on the GPU (device)
// but is called from the CPU (host). It must have a void return type.
...
int main()
{
    cout << "Hello from the CPU, about to launch the kernel." << endl;
